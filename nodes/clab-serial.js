/**
 * CompuLab Serial Node für Node-RED
 * RS232/RS485 Kommunikation für IOT-GATE-iMX8, SBC-IOT-iMX8
 */
...
module.exports = function(RED) {
    ...
    // ============================================
    // Serial Config Node - Gemeinsame Konfiguration
    // ============================================
    function ClabSerialConfigNode(config) {
        ...
        node.openPort = async function() {
            ...
            try {
                // UART Modus setzen wenn Backpanel Port
    ...
    // ============================================
    // Serial In Node - Empfängt serielle Daten
    // ============================================
    function ClabSerialInNode(config) {
        ...
        node.status({ fill: 'yellow', shape: 'dot', text: 'Verbinde...' });
            ...
            try {
                ...
                if (node.outputFormat === 'buffer') {
                    ...
                    if (node.delimiter) {
                        ...
                        buffer = lines.pop(); // Letztes unvollständiges behalten
                ...
                node.status({ fill: 'green', shape: 'dot', text: 'Empfangen' });
            } catch (err) {
    ...
    // ============================================
    // Serial Out Node - Sendet serielle Daten
    // ============================================
    function ClabSerialOutNode(config) {
        ...
        node.on('input', async (msg, send, done) => {
            try {
                ...
                // In Buffer konvertieren wenn nötig
    ...
    // ============================================
    // UART Mode Node - Setzt RS485/RS232 Modus
    // ============================================
    function ClabUartModeNode(config) {
        ...
        node.status({ fill: 'blue', shape: 'dot', text: `Modus: ${node.mode}` });

        node.on('input', async (msg, send, done) => {
            const mode = msg.payload || node.mode;
