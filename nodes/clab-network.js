 ...
 * Konsolidierter Node für Netzwerk-Diagnose
 */
            ...
            try {
                ...
                switch (action) {
                    case 'ping':
                        ...
                        if (!pingHost) throw new Error('Host erforderlich');
                        ...
                        node.status({ fill: result.alive ? 'green' : 'red', shape: 'dot', text: result.alive ? `${result.rtt.avg?.toFixed(1)}ms` : 'Nicht erreichbar' });
                        break;
                        
                    case 'dns':
                        const hostname = msg.hostname || msg.payload?.hostname || msg.payload || config.hostname;
                        if (!hostname) throw new Error('Hostname erforderlich');
                        node.status({ fill: 'blue', shape: 'ring', text: `DNS ${hostname}...` });
                        result = await network.dnsLookup(hostname, { type: msg.type || config.dnsType || 'A' });
                        node.status({ fill: result.success ? 'green' : 'yellow', shape: 'dot', text: result.records[0] || 'Keine Einträge' });
                        break;
                        
                    case 'reverse-dns':
                        const ip = msg.ip || msg.payload?.ip || msg.payload;
                        if (!ip) throw new Error('IP erforderlich');
                        result = await network.reverseDns(ip);
                        node.status({ fill: result.success ? 'green' : 'yellow', shape: 'dot', text: result.hostnames[0] || 'Nicht gefunden' });
                        break;
                        
                    case 'traceroute':
                        const traceHost = msg.host || msg.payload?.host || config.host;
                        if (!traceHost) throw new Error('Host erforderlich');
                    ...
                    case 'port-check':
                        ...
                        if (!portHost || !port) throw new Error('Host und Port erforderlich');
                        ...
                        node.status({ fill: result.open ? 'green' : 'red', shape: 'dot', text: result.open ? `Offen (${result.responseTime}ms)` : 'Geschlossen' });
                        break;
                    ...
                    case 'http-check':
                        ...
                        node.status({ fill: result.ok ? 'green' : 'red', shape: 'dot', text: result.statusCode ? `${result.statusCode} (${result.responseTime}ms)` : 'Fehler' });
                        break;
                        
                    case 'routes':
                        result = await network.getRoutes();
                        node.status({ fill: 'green', shape: 'dot', text: `${result.count} Routen` });
                        break;
                        
                    case 'arp':
                        result = await network.getArpTable();
                        node.status({ fill: 'green', shape: 'dot', text: `${result.count} Einträge` });
                        break;
                        
                    case 'speed-test':
                        node.status({ fill: 'blue', shape: 'ring', text: 'Speed Test...' });
                        result = await network.speedTest({ url: msg.url || config.speedTestUrl });
                        node.status({ fill: result.success ? 'green' : 'red', shape: 'dot', text: result.success ? `${result.speedMbps} Mbps` : 'Fehler' });
                        break;
